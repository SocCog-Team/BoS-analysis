function RampingPERtrial_SimulatedSubjects()
...
%% Simulating subjects with same ramping rate per trial (0.1/10 = 0.01) but different learning time

InitialAddedUpRamping =  0.01 %subject improves by adding this probability to high reward probbaility at each step of learning time (DeltaTime)
DeltaTrials = 1
...
DELTATIME_Range = InitialDeltaTime: 10: 100
...
for DT = 1 : numel(DELTATIME_Range)
    ...
    for RAMP = 1 : MaxRampNum-1
        B = []
        B = binornd(1,0.5+AddedUpRamping,[1,DeltaTrials]);
        A = [A,B]
...
legend('learning time: after 10 trials','learning time: after 100 trials')
title("subject's choice, smoothed over 50 data points window")
xlabel('trial number')
ylabel('choice')
sgtitle('ramping rate: 0.01')
%% simulating subjects with same learning time but different ramping rate
DELTATIME = 50
RampingRange = (0.1:0.1:0.5)./DELTATIME
% RampingRange = 0.1:0.1:0.5
...
for RR = 1 : numel(RampingRange)
    ...
    for RAMP = 1 : numel(AddedUpRamping)
        B = []
        B = binornd(1,AddedUpRamping(RAMP),[1,1]);
        A = [A,B]
    end
    A = [A,ones(1,nTrials-numel(A))] %after fully learning, subject always choose the best option
...
xlabel('trial number')
ylabel('choice')
title("subject's choice, smoothed over 70 data points window")
legend('ramping rate = 0.002','ramping rate = 0.01')
sgtitle('learning time: after 50 trials, different ramping rate per trial')
...
sgtitle('fixed ramping rate: 0.01')
