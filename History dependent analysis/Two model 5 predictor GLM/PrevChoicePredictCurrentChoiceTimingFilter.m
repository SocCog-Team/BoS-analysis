...
%% defining switching vetors: timed defined all trials (red curve)
  ...
  %% defining switching vetors: At switch timw defined (pink curve)
                ...
                for Turn = 1: 3
                    ...
                    if AC == 1
                       ...
                       PREDICTORpartnerChoice = arrayfun(convertZeroOne,squeeze(HistoryAndTimingPredictor(:,:,Turn,1)));
                        ...
                        SelfChoiceHistoryCoefs{AC}(:,idata,Turn) = table2array(SelfChoiceHistoryModel.Coefficients(2:end,1));
                        
                        PartnerChoiceHistoryModel = fitglm(PREDICTORpartnerChoice,RESPONSE,'linear','Distribution','binomial','Link',LinkFunction);
                        PartnerChoiceHistoryCoefs{AC}(:,idata,Turn) = table2array(PartnerChoiceHistoryModel.Coefficients(2:end,1));
                end
            end
    ...
    for i = 1 : BeforeAfter_Length
      ...
      for AC = 1 : 2
                MeanSelfChoiceHistoryCoefs{AC} = squeeze(mean(SelfChoiceHistoryCoefs{AC}(:,:,:),2,'omitnan'));
                MeanPartnerChoiceHistoryCoefs{AC} = squeeze(mean(PartnerChoiceHistoryCoefs{AC}(:,:,:),2,'omitnan'));
