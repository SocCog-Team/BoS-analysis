  ...
  %% defining switching vetors: At switch timw defined (pink curve)
            ...
            for AC = 1:2
                for Turn = 1: 3
                    ...
                    PREDICTOR_CrossSelfOther = AtSwitch_SelfotherActor_PrevSwitchesPartner{AC,Turn};
                    PREDICTOR_CrossOtherSelf =  AtSwitch_OtherSelfActor_PrevSwitchesPartner{AC,Turn};
                    if size(PREDICTORselfOther,1)>1
                        SelfOtherModel = fitglm(PREDICTORselfOther,ones(size(PREDICTORselfOther,1),1),'linear','Distribution','binomial','Link',LinkFunction);
                    ...
                    if size(PREDICTORotherSelf,1)>1
                        OtherSelfModel = fitglm(PREDICTORotherSelf,ones(size(PREDICTORotherSelf,1),1),'linear','Distribution','binomial','Link',LinkFunction);
                    ...
                    if size(PREDICTOR_CrossSelfOther,1)>1
                        SelfotherActor_PrevSwitchPartnerModel = fitglm(PREDICTOR_CrossSelfOther,ones(size(PREDICTOR_CrossSelfOther,1),1),'linear','Distribution','binomial','Link',LinkFunction);
                    ...
                    if size(PREDICTOR_CrossOtherSelf,1)>1
                        OtherSelfActor_PrevSwitchPartnerModel = fitglm(PREDICTOR_CrossOtherSelf,ones(size(PREDICTOR_CrossOtherSelf,1),1),'linear','Distribution','binomial','Link',LinkFunction);
    ...
    for i = 1 : BeforeAfter_Length
      ...
      figure
            ...
            legend('Bfirst','Bsimul','Bsecond')
            sgtitle(strcat(A_Name,'-',B_Name))
end
