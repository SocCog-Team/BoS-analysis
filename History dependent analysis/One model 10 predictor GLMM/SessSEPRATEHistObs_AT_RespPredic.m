    ...
    for idata = 1 : length(MergedData)
        ...
        for AC = 1:2
            ...
            if exist('SelFChoiceHist_PartnerChoiceHist_ObservedHist_Model','var')
                ...
                if AC == 1
                    subtitle(strcat(A_Name,',Pairwise Correlations between Predictors: Model failed due to high Multicollinearity among predictors'))
                else
                    subtitle(strcat(B_Name,',Pairwise Correlations between Predictors: Model failed due to high Multicollinearity among predictors'))
        ...
        SessionDate = string(AllDates_NotRepeat{idata})
        title(strcat(A_Name,'-',B_Name,'-','Session: ',SessionDate))

        filename = strcat(A_Name,'-',B_Name,'-',SessionDate,'FCOmodelperf.jpg')
        ax = gcf;
