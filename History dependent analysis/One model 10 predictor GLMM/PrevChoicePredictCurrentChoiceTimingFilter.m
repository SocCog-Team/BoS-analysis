...
%% finding turns ids
 ...
 TrialsVector = (1 : size(HistoryVector,1))';
            ...
            for AC = 1:2
                for Turn = 1: 3
                    PREDICTORselfChoice = []
                    PREDICTORselfChoice = squeeze(HistoryAndTimingPredictor(:,:,Turn,AC));
                    RESPONSE = []
                    RESPONSE = ChoiceResponse(:,Turn,AC); %In both predictor and response, 1 means preferred color
                    PREDICTORpartnerChoice = []
                    if AC == 1
                       PREDICTORpartnerChoice = arrayfun(convertZeroOne,squeeze(HistoryAndTimingPredictor(:,:,Turn,2)));
                    else
                       PREDICTORpartnerChoice = arrayfun(convertZeroOne,squeeze(HistoryAndTimingPredictor(:,:,Turn,1)));
                    end
                    ...
                    ChoiceHistoryCoefs{AC}(:,idata,Turn) = double(SelChoiceHistory_PartnerChoiceHistyory_Model.Coefficients(2:end,2));
                    ChoiceHistoryGoodnessOfFit{AC}(idata,:,Turn) = double(SelChoiceHistory_PartnerChoiceHistyory_Model.Coefficients(2:end,6));
    ...
    for AC = 1 : 2
        MeanAcrossSessChoiceHistoryCoefs{AC} = mean(ChoiceHistoryCoefs{AC}(:,:),2,'omitnan');
        MeanAcrossSessChoiceHistoryGoodnessPval{AC}(fol,:) = mean(ChoiceHistoryGoodnessOfFit{AC}(:,:),1,'omitnan');
            ...
            for AC = 1:2
                if AC == 1
                   ...
                   for Turn = 1:3
                       ...
                       plot(1:BeforeAfter_Length,flipud(MeanSelfChoiceHistoryCoefs{AC}(:,Turn)),'o-','Color',Col,'LineWidth',LW)
                   end
                   ylabel('GLM Weight')
                   xticks(1:5)
                   xticklabels(string(xtickname))
                   xtickangle(45)
                   title('Self Choice History Model')
                   ...
                   for Turn = 1:3
                       ...
                       plot(1:BeforeAfter_Length,flipud(MeanPartnerChoiceHistoryCoefs{AC}(:,Turn)),'o-','Color',Col,'LineWidth',LW)
                   end
                   ylabel('GLM Weight')
                   xticks(1:5)
                   xticklabels(string(xtickname))
                   ...
                   title('Partner Choice History Model')
