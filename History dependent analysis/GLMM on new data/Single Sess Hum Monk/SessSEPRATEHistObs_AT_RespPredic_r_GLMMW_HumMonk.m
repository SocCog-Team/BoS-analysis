...
for fol = 1:length(MonkeyFoldersName)
    ...
    NeededDataString = 'JointTrials';
    NeededDataIndex = contains(valid_matfile_list,NeededDataString)
    valid_matfile_list = valid_matfile_list(NeededDataIndex)
    ...
    switch ActionTimingMeasure
        ...
        for AC = 1:2
            ...
            if exist('SelFChoiceHist_PartnerChoiceHist_ObservedHist_Model','var')
                ...
                if AC == 1
                    subtitle(ax1,strcat(A_Name,' r= ',string(round(r_corrcoff,2)),' p val = ',string(round(r_pval,4)),' smoothed r= ',string(round(r_smoothed,2))))
                else
                    subtitle(ax1,strcat(B_Name,' r= ',string(round(r_corrcoff,2)),' p val = ',string(round(r_pval,4)),' smoothed r= ',string(round(r_smoothed,2))))
        ...
        SessionDate = string(AllDates_NotRepeat{idata})
        title(ax1,strcat(A_Name,'-',B_Name,'-','Session: ',SessionDate))

        filename = strcat(A_Name,'-',B_Name,'-',SessionDate,'FCOglmWeights.pdf')
        ax = gcf;
