...
for MONKEY = 1 : numel(MonkeysName)
    if   exist(sprintf('NHP_SoloA%s_fullPath',MonkeysName{MONKEY})) && exist(sprintf('NHP_SoloB%s_fullPath',MonkeysName{MONKEY})) == 2
        ...
        parfor SessSolo =  1 : numel(NHP_SoloB)
            ...
            if numel(RewardedID)>TrialLimit
                ...
                choices = ChoiceVectorRewarded(1:TrialLimit+1); % choice vector of Agent A or B,  1 = preferred colour  0 = non prefered colour,
                ...
                feedbacks = RewardValueVectorRewarded(1:TrialLimit+1);% reward value vector for each Agent from pay off matrix???
        ...
        AllSessinsDates = AllSessinsDates(~isnan(AllSessinsDates))
