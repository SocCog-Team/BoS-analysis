...
SimulGoSig_DividingThreshold = 100; %when based on Go signal, players are assumed to act simul, but still we can categorise one of them as faster based on AT,
%This threshold is for categorizing AT in the case of simul Go sig

TrialThreshold = 100 %less than this number of trials cant be analysed
...
for fol = 1:length(MonkeyFoldersName)
    ...
    %     % (NotReapetetive_NHP_SessionsIdx,:)
    ...
    for idata = 1 : length(MergedData)
        ...
        if ~exist('BlockedOrShuffledStruct')
            ...
            A_Name = cur_session_id_struct_arr(idata).subject_A;
            B_Name = cur_session_id_struct_arr(idata).subject_B;
            ...
            FigName = strcat(A_Name,'-',B_Name,SubTypeTrialString)
            ...
            filename = []
            filename = strcat(A_Name,'-',B_Name,'-',SessionDate,SubTypeTrialString,'.jpg')
            sgtitle(strcat(A_Name,'-',B_Name,SessionDate,SubTypeTrialString))
            ax = gcf;
            exportgraphics(ax,sprintf(filename),'Resolution',600)


            %% other player switches
            FigName = strcat(A_Name,'-Bswitch',B_Name,SubTypeTrialString)
            figure('Name',sprintf(FigName), 'NumberTitle', 'off','Position',[488,49.800000000000004,500.2,732.8000000000001])
            for Turn = 1 : 3
                if Turn == 1
                    subplot(3,2,1)
            ...
            filename = []
            filename = strcat(A_Name,'-',B_Name,'-',SessionDate,SubTypeTrialString,'SwitchB.jpg')
            sgtitle(strcat(A_Name,'-',B_Name,'SwitchB',SessionDate,SubTypeTrialString))
            ax = gcf;
            exportgraphics(ax,sprintf(filename),'Resolution',600)
        else
            for ii = 1 : I
