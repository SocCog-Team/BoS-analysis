...
for fol = 1:length(MonkeyFoldersName)
    ...
    for idata = 1 : length(MergedData)
        ...
        if length(BeforeShedding)>1
            for i_struct = 1 : length(BeforeShedding)
                ...
                if i_struct == 1
                    ...
                    WholeTrialsChoiceVe ctorA = TransientData.FullPerTrialStruct.PreferableTargetSelected_A;
        ...
        switch ActionTimingMeasure
            case 'RT'
                ...
                tmp_AgoB = diffRT_AB < (- TimeRange_ToleranceThreshold);
                tmp_BgoA = diffRT_AB > ( TimeRange_ToleranceThreshold);
    ...
    for idata = 1 : length(MergedData)
        ...
        sgtitle(strcat(A_Name,'-',B_Name,SessionDate))
        ax = gcf;
        ...
        filename = []
        filename = strcat(A_Name,'-',B_Name,'-',SessionDate,'SwitchB.jpg')
        sgtitle(strcat(A_Name,'-',B_Name,'SwitchB',SessionDate))
        ax = gcf;
        exportgraphics(ax,sprintf(filename),'Resolution',600)
