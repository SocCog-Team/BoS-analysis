function [Statitical_Results] = bosa_RT_analysis_one_session_SoloASemiSolo_CuriusA_HumanB(DataFileName)

% DataFilePath = '20230705T134053.A_Curius.B_RS.SCP_01.triallog.v017'
load(DataFileName);
...
ytix = get(gca, 'YTick');
set(gca, 'YTick',ytix, 'YTickLabel',ytix*100);
legend(strcat('monkey(curius)',' was first'),'','Actor B was second',''); % for the legend, name of Actor_A is printed
ylabel('% of trials');
subtitletxt = {strcat('monkey(curius)',':','trial number =',string(numel(RT_A_ms_AllTrials(Turn_ActorA_First_Rewarded_AND_SemiSolo_ID)))); ...
    strcat('human:','trial number =',string(numel(RT_B_ms_AllTrials(Turn_ActorB_Second_Rewarded_AND_SemiSolo_ID))))};
...
ylabel('% of trials');
subtitletxt = {strcat('monkey(curius)',' and human:','trial number =',string(numel(RT_A_ms_AllTrials(Turn_ActorA_Simul_Rewarded_AND_SemiSolo_ID))))};
subtitle(subtitletxt);
% pbaspect([1 1 1])
%%
sh(3) = subplot(2,3,3);
...
ytix = get(gca, 'YTick');
set(gca, 'YTick',ytix, 'YTickLabel',ytix*100);
legend(strcat('monkey(curius)',' was second'),'','human was first',''); % for the legend, name of Actor_A is printed
ylabel('% of trials');
subtitletxt = {strcat('monkey(curius)',':','trial number =',string(numel(RT_A_ms_AllTrials(Turn_ActorA_Second_Rewarded_AND_SemiSolo_ID))));strcat('human:','trial number =', ...
...
annotation('textbox',...
    ...
    'String',{'Semi-Solo'},...
    'FitBoxToText','off');
sgtitle('Session with only Semi-Solo and Solo (monkey and human)')
