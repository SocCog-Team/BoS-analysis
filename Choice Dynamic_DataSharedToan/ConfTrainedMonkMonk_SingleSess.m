...
for fol = 1:length(ConditionFoldersName)
    ...
    % NumberOfTimePoints = 2;  % we want to look at two time points, first and last periods of training
     ...
     for idata = 1 : NumberOfTimePoints
            ColorChoice = [];
            RTA = [];
...
%% defining switching vetors: timed defined all trials (red curve)
            ...
            for AC = 1 : 2
                for Turn = 1 : 3
                    ...
                    MeanSelfOther(Turn,:,AC,idata) = mean(SelfOther_NeededSwitchVector,'omitnan');
                    MeanOtherSelf(Turn,:,AC,idata) = mean(OtherSelf_NeededSwitchVector,'omitnan');
            ...
            %% CRISS CROSS : When actor A switches, what happen to actor B and vice versa
             for AC = 1 : 2
                for Turn = 1 : 3
                    ...
                    CrossMeanSelfOther(Turn,:,AC,idata) = mean(SelfOther_NeededSwitchVector,'omitnan');
                    CrossMeanOtherSelf(Turn,:,AC,idata) = mean(OtherSelf_NeededSwitchVector,'omitnan');
  ...
  %% defining switching vetors: At switch timw defined (pink curve)
            ...
            for AC = 1 : 2
                for Turn = 1 : 3
                    ...
                    AtSwitch_MeanSelfOther(Turn,:,AC,idata) = mean(SelfOther_NeededSwitchVector,'omitnan');
                    AtSwitc_MeanOtherSelf(Turn,:,AC,idata) = mean(OtherSelf_NeededSwitchVector,'omitnan');
            ...
            for AC = 1 : 2
                for Turn = 1 : 3
                    ...
                    CrossAtSwitch_MeanSelfOther(Turn,:,AC,idata) = mean(SelfOther_NeededSwitchVector,'omitnan');
                    CrossAtSwitc_MeanOtherSelf(Turn,:,AC,idata) = mean(OtherSelf_NeededSwitchVector,'omitnan');
...
for idata = 1 : NumberOfTimePoints
    ...
    figure('Name',sprintf(FigName), 'NumberTitle', 'off','Position',[1,49,1536,740.8])
    for Turn = 1 : 3
             if Turn == 1
                 subplot(3,2,1)
                 hold on
    %                 %% blue curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,MeanOverSessSelfOther(Turn,:,1),'.-','Color',ActorAcolor,'LineWidth',4)
    ...
    %                 %% red curve
                      plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessSelfOther(Turn,:,2),'.-','Color',ActorBcolor,'LineWidth',4)
    ...
    % %                 %% pink curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessAtSwitch_MeanSelfOther(Turn,:,2),'.-','Color',ActorBAtSwitchCol,'LineWidth',4)
    ...
    %%
                    ...
                    hold on
                    %                 %% blue curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,MeanOverSessOtherSelf(Turn,:,1),'.-','Color',ActorAcolor,'LineWidth',4)
    ...
    %                 %% red curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessOtherSelf(Turn,:,2),'.-','Color',ActorBcolor,'LineWidth',4)
    ...
    %                 %% pink curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessAtSwitch_MeanOtherSelf(Turn,:,2),'.-','Color',ActorBAtSwitchCol,'LineWidth',4)
    ...
    if idata == 1
    sgtitle(strcat('Pair: ',sprintf(A_Name),', ',sprintf(B_Name),' two first sessions', ',RT range: 100ms'))
    else
    sgtitle(strcat('Pair: ',sprintf(A_Name),', ',sprintf(B_Name),' two last sessions',',RT range: 100ms'))
    end

    annotation('textbox',...
    [0.0359375 0.747380129048942 0.0593750012603899 0.0361771063722236],...
    'String',{'Actor B first'});

    annotation('textbox',...
    [0.0421875 0.460123109610496 0.0656250014094015 0.0361771063722236],...
    'String',{'Actor B simul'});


    annotation('textbox',...
    [0.0354166666666666 0.18042548542043 0.0729166682499151 0.0361771063722236],...
    'String',{'Actor B second'});
    if idata == 2
        filename = []
        filename = strcat(A_Name,'-',B_Name,'.jpg')
        ax = gcf;
    ...
    for Turn = 1 : 3
             if Turn == 1
                 ...
                 hold on
    %                 %% blue curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,MeanOverSessSelfOther(Turn,:,2),'.-','Color',ActorBcolor,'LineWidth',4)
                     % 
                     % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,ci_lowerSelfOther(Turn,:,2),'-b','LineWidth',0.5)
                     % 
                     % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,ci_upperSelfOther(Turn,:,2),'-b','LineWidth',0.5)
                     % 
                     % fill([1:BeforeAfter_Length+1+BeforeAfter_Length, fliplr(1:BeforeAfter_Length+1+BeforeAfter_Length)], [ci_lowerSelfOther(Turn,:,2), fliplr(ci_upperSelfOther(Turn,:,2))],'b', 'FaceAlpha', 0.3,'EdgeColor','none');
    %                 %% red curve
                      plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessSelfOther(Turn,:,1),'.-','Color',ActorAcolor,'LineWidth',4)
     
    %                  plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_lowerSelfOther(Turn,:,1),'-r','LineWidth',0.5)
    % 
    %                  plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_upperSelfOther(Turn,:,1),'-r','LineWidth',0.5)
    % % 
    %                  fill([1:BeforeAfter_Length+1+BeforeAfter_Length, fliplr(1:BeforeAfter_Length+1+BeforeAfter_Length)], [Cross_ci_lowerSelfOther(Turn,:,1), fliplr(Cross_ci_upperSelfOther(Turn,:,1))],'r', 'FaceAlpha', 0.3,'EdgeColor','none');
    % %                 %% pink curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessAtSwitch_MeanSelfOther(Turn,:,1),'.-','Color',ActorAatSwitchCol,'LineWidth',4)
    
                    % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_lowerAtSwitchSelfOther(Turn,:,1),'-m','LineWidth',0.5)
                    % 
                    % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_upperAtSwitchSelfOther(Turn,:,1),'-m','LineWidth',0.5)
                    % 
                    % fill([1:BeforeAfter_Length+1+BeforeAfter_Length, fliplr(1:BeforeAfter_Length+1+BeforeAfter_Length)], [Cross_ci_lowerAtSwitchSelfOther(Turn,:,1), fliplr(Cross_ci_upperAtSwitchSelfOther(Turn,:,1))],'m', 'FaceAlpha', 0.3,'EdgeColor','none');
    ...
    %%
                    ...
                    hold on
                    %                 %% blue curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,MeanOverSessOtherSelf(Turn,:,2),'.-','Color',ActorBcolor,'LineWidth',4)
     
                     % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,ci_lowerOtherSelf(Turn,:,2),'-b','LineWidth',0.5)
                     % 
                     % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,ci_upperOtherSelf(Turn,:,2),'-b','LineWidth',0.5)
                     % 
                     % fill([1:BeforeAfter_Length+1+BeforeAfter_Length, fliplr(1:BeforeAfter_Length+1+BeforeAfter_Length)], [ci_lowerOtherSelf(Turn,:,2), fliplr(ci_upperOtherSelf(Turn,:,2))],'b', 'FaceAlpha', 0.3,'EdgeColor','none');
    %                 %% red curve
                      plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessOtherSelf(Turn,:,1),'.-','Color',ActorAcolor,'LineWidth',4)
    % 
    %                  plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_lowerOtherSelf(Turn,:,1),'-r','LineWidth',0.5)
    % 
    %                  plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_upperOtherSel(Turn,:,1),'-r','LineWidth',0.5)
    % % 
    %                  fill([1:BeforeAfter_Length+1+BeforeAfter_Length, fliplr(1:BeforeAfter_Length+1+BeforeAfter_Length)], [Cross_ci_lowerOtherSelf(Turn,:,1), fliplr(Cross_ci_upperOtherSel(Turn,:,1))],'r', 'FaceAlpha', 0.3,'EdgeColor','none');
    %                 %% pink curve
                     plot(1:BeforeAfter_Length+1+BeforeAfter_Length,CrossMeanOverSessAtSwitch_MeanOtherSelf(Turn,:,1),'.-','Color',ActorAatSwitchCol,'LineWidth',4)
    
                    % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_lowerAtSwitchOtherSelf(Turn,:,1),'-m','LineWidth',0.5)
                    % 
                    % plot(1:BeforeAfter_Length+1+BeforeAfter_Length,Cross_ci_upperAtSwitchOtherSelf(Turn,:,1),'-m','LineWidth',0.5)
                    % 
                    % fill([1:BeforeAfter_Length+1+BeforeAfter_Length, fliplr(1:BeforeAfter_Length+1+BeforeAfter_Length)], [Cross_ci_lowerAtSwitchOtherSelf(Turn,:,1), fliplr(Cross_ci_upperAtSwitchOtherSelf(Turn,:,1))],'m', 'FaceAlpha', 0.3,'EdgeColor','none');
   ...
   % exportgraphics(ax,figname,'Resolution',600)
    ...
    if idata == 2
        filename = []
        filename = strcat(A_Name,'-',B_Name,'Bswitch.jpg')
        ax = gcf;
        exportgraphics(ax,sprintf(filename),'Resolution',600)
    end
